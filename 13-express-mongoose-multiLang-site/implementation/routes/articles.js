...
module.exports = () => {
  ...
  router.get('/new', isAuthenticated, (req, res) => {
    return res.render('article-form', { article: {}, action: `/${res.locals.lang}/articles/new` });
  });
  ...
  router.post('/new', isAuthenticated, async (req, res) => {
    try {
      ...
      return res.redirect(`/${res.locals.lang}/articles`);
    }
    catch (err) {
      if (err.name === 'ValidationError') {
        ...
        return res.render('article-form', {
          errors,
          article: req.body,
          action: `/${res.locals.lang}/articles/new`
        });
      }
  ...
  router.get('/:url/edit', isAuthenticated, async (req, res) => {
    try {
      ...
      return res.render('article-form', { article, action: `/${res.locals.lang}/articles/${article.url}/edit` });
    }
    catch (err) {
      console.error(err);
      return res.status(500).send('server error');
